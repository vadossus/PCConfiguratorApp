...
const UIManager = (() => {
    ...
    const _apply_filters_and_render = () => {
        const track = document.getElementById('community-builds-track');
        if (!track || !_all_builds.length) return;

        const filterType = document.getElementById('filter-build-type')?.value || 'all';
        const sortPrice = document.getElementById('sort-build-price')?.value || 'default';

        let filtered = _all_builds.filter(b => {
            if (filterType === 'all') return true;
            return b.build_type === filterType;
        });

        if (sortPrice === 'asc') {
            filtered.sort((a, b) => floatValue(a.total_price) - floatValue(b.total_price));
        } else if (sortPrice === 'desc') {
            filtered.sort((a, b) => floatValue(b.total_price) - floatValue(a.total_price));
        }
        window.user_build_data = filtered;

        if (!filtered.length) {
            track.innerHTML = '<div class="no-builds">Сборок с такими параметрами не найдено</div>';
            _carousel_index = 0;
            _update_carousel();
            return;
        }
        ...
        filtered.forEach(b => {
            ...
            if (main_img) {
                html += `<img src="${main_img}" loading="lazy" onerror="this.src='source/icons/default.svg'">`;
            } else {
            ...
            if (previews.length === 1) {
                html += `<div class="thumb tall"><img src="${previews[0]}" loading="lazy" onerror="this.style.display='none'"></div>`;
            } else if (previews.length === 2) {
                html += `<div class="thumb tall"><img src="${previews[0]}" loading="lazy" onerror="this.style.display='none'"></div>`;
                html += `<div class="thumb tall"><img src="${previews[1]}" loading="lazy" onerror="this.style.display='none'"></div>`;
            } else if (previews.length >= 3) {
                html += `<div class="thumb tall"><img src="${previews[0]}" loading="lazy" onerror="this.style.display='none'"></div>`;
                html += '<div class="thumb-row">';
                html += `<div class="thumb small"><img src="${previews[1]}" loading="lazy" onerror="this.style.display='none'"></div>`;
                html += `<div class="thumb small"><img src="${previews[2]}" loading="lazy" onerror="this.style.display='none'"></div>`;
    ...
    const _update_carousel = () => {
        const track = document.getElementById('community-builds-track');
        const next_btn = document.getElementById('next-btn');
        const prev_btn = document.getElementById('prev-btn');

        if (!track || !next_btn || !prev_btn) return;
        if (!track.children.length || track.querySelector('.no-builds')) {
        ...
        _carousel_max = Math.max(0, track.children.length - visible);

        if (_carousel_index > _carousel_max) {
            _carousel_index = _carousel_max;
        }

        track.style.transform = `translateX(-${_carousel_index * (card_width + gap)}px)`;
        prev_btn.style.opacity = _carousel_index === 0 ? '0.5' : '1';
        next_btn.style.opacity = _carousel_index >= _carousel_max ? '0.5' : '1';
    };

    const _init_carousel_events = () => {
        const next_btn = document.getElementById('next-btn');
        const prev_btn = document.getElementById('prev-btn');
        ...
        window.addEventListener('resize', _update_carousel);
    };
    ...
    const _like_build = async (id) => {
        try {
            ...
            if (data.success) {
                ...
                const build = _all_builds.find(b => b.id == id);
                if (build) build.likes = data.likes;
    ...
    const _init = () => {
        ...
        _init_carousel_events();

        const typeSelect = document.getElementById('filter-build-type');
        const priceSort = document.getElementById('sort-build-price');

        if (typeSelect) typeSelect.addEventListener('change', _apply_filters_and_render);
        if (priceSort) priceSort.addEventListener('change', _apply_filters_and_render);
